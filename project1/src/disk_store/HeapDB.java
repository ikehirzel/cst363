...
public class HeapDB implements DB, Iterable<Record> {
		...
		if (blockNum > 1 && blockNum <= bf.getLastBlockIndex()) {
			...
			if (recNum >= 0) {
				...
				for (int i = 0; i < indexes.length; i++) {
					if (indexes[i] != null) {
						if (!(rec.get(i) instanceof IntField)) {
							throw new UnsupportedOperationException("Expected field to be of type IntField");
						}
						
						IntField field = (IntField) rec.get(i);
						indexes[i].insert(field.getValue(), blockNum);
					}
				}
	...
	public List<Record> lookup(String fname, int key) {
		...
		if (indexes[fieldNum] == null) { 
			...
			for (Record rec : this) { 
				if (!(rec.get(fieldNum) instanceof IntField)) {
					throw new UnsupportedOperationException("Expected field to be of type IntField");
				}
		...
		for (int blockNum = bitmapBlock + 1; blockNum <= bf.getLastBlockIndex(); blockNum++) {
			...
			for (int recNum = 0; recNum < recMap.size(); recNum++) {
				if (recMap.getBit(recNum)) {
					...
					IntField field = (IntField) rec.get(fieldNum);					
					index.insert(field.getValue(), blockNum);
				}
			}
		}
	}
