...
class TableTest {
	...
	void serializeDeserialize() {
		...
		Tuple t1 = newInstTuple("11111", "Molina", "Music", 70000, instSchema);
		ByteBuffer buf = ByteBuffer.allocate(128);
		t1.serialize(buf, 0);

		// create a record of the same type, and deserialize it from the buffer
		Tuple t2 = newInstTuple("12345", "Endive", "Finance", 50000, instSchema);
	...
	@Test
	void serializeDeserializeBytes() {
		// like serializeDeserialize but with a byte array instead of a ByteBuffer
		Tuple t1 = newInstTuple("11111", "Molina", "Music", 70000, instSchema);
		byte[] buf = new byte[128];
		t1.serialize(buf, 10);

		Tuple t2 = newInstTuple("12345", "Endive", "Finance", 50000, instSchema);
		t2.deserialize(buf, 10);
	...
	@Test
	void blockInsert() {
		Block b = new Block(instSchema);
		assertTrue(b.size() == 0);
		
		Tuple t1 = newInstTuple("11111", "Molina", "Music", 70000, instSchema);
		Tuple t2 = newInstTuple("12345", "Endive", "Finance", 50000, instSchema);
		Tuple t3 = newInstTuple("12346", "Freens", "Comp. Sci.", 65000, instSchema);
		b.insert(t1);
		b.insert(t2);
		assertTrue(b.size() == 2);
	...
	void blockDelete() {
		...
		b.delete("name", new VarcharField("Molina"));
		assertTrue(b.size() == 2);
		
		b.delete("name", new VarcharField("Votsak"));
		assertTrue(b.size() == 2);
	}
	
	@Test
	void blockDeleteInsert() {
		Block b = new Block(instSchema);
		Tuple t1 = newInstTuple("11111", "Molina", "Music", 70000, instSchema);
		Tuple t2 = newInstTuple("12345", "Endive", "Finance", 50000, instSchema);
		Tuple t3 = newInstTuple("12346", "Freens", "Comp. Sci.", 65000, instSchema);
		Tuple t4 = newInstTuple("43432", "Erkle",  "Comp. Sci.", 85000, instSchema);
		b.insert(t1);
		b.insert(t2);
		b.insert(t3);
		
		b.delete("dept_name", new VarcharField("Biology"));
		assertTrue(b.size() == 3);
		
		b.delete("ID", new VarcharField("12346"));
		assertTrue(b.size() == 2);
	...
	@Test
	void blockInsertKeyConstraint() {
		// set primary key of instructor table
		instSchema.setKey(Arrays.asList("ID"));
		...
		Tuple t2 = newInstTuple("12345", "Endive", "Finance", 50000, instSchema);
		Tuple t3 = newInstTuple("12346", "Freens", "Comp. Sci.", 65000, instSchema);
		Tuple t4 = newInstTuple("11111", "Erkle",  "Comp. Sci.", 85000, instSchema);
		b.insert(t1);
		b.insert(t2);
		b.insert(t3);
		
		Assertions.assertThrows(IllegalArgumentException.class, () -> b.insert(t4));		
	}
}
